...
Install_Nginx()
{
    ...
    if [ "${Default_Website_Dir}" != "/ecmoban/wwwroot/default" ]; then
        sed -i "s#/ecmoban/wwwroot/default#${Default_Website_Dir}#g" /usr/local/nginx/conf/nginx.conf
    fi

    if [ "${Stack}" = "lnmp" ]; then
        cat >${Default_Website_Dir}/public/.user.ini<<EOF
open_basedir=${Default_Website_Dir}/../:/tmp/:/proc/
EOF
        chmod 644 ${Default_Website_Dir}/public/.user.ini
